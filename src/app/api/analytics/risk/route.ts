...
export async function GET(request: NextRequest) {
  try {
    ...
    const [locations, safetyScore] = await Promise.all([
      analyticsService.locations(organizationId, filters),
      analyticsService.safetyScore(organizationId, filters),
    ]);
    return apiSuccess({ data: { locations, safetyScore } });
  } catch (error) {
