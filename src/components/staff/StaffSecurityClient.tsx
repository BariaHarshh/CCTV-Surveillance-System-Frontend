  ...
  return (
    <StaffShell user={user}>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <button type="button" onClick={revokeOthers} className="rounded-full border border-white/[0.08] px-5 py-2 text-sm">Logout Other Sessions</button>
      </div>

      <section className="mt-8 rounded-2xl border border-white/[0.08] bg-surface/50 p-6">
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-white/[0.08]">
        <h2 className="border-b border-white/[0.06] bg-surface/50 px-4 py-3 font-semibold">Active Sessions</h2>
        {loading ? (
          <div className="p-4"><div className="h-20 animate-pulse rounded-xl bg-white/[0.04]" /></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px] text-left text-sm">
              <thead><tr className="border-b border-white/[0.06] text-xs text-muted"><th className="px-4 py-3">Device</th><th className="px-4 py-3">Last Active</th><th className="px-4 py-3">Status</th><th className="px-4 py-3" /></tr></thead>
              <tbody>
                {sessions.map((s) => (
