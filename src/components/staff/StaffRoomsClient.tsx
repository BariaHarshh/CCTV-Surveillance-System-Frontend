  ...
  return (
    <StaffShell user={user}>
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-white/[0.08]">
        {loading ? <div className="p-6"><div className="h-32 animate-pulse rounded-xl bg-white/[0.04]" /></div> : rooms.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px] text-left text-sm">
              <thead><tr className="border-b border-white/[0.06] text-xs text-muted"><th className="px-4 py-3">Room</th><th className="px-4 py-3">Code</th><th className="px-4 py-3">Building</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Capacity</th><th className="px-4 py-3">Status</th></tr></thead>
              <tbody>
                {rooms.map((r) => (
