  ...
  return (
    <AdminShell user={user}>
      ...
      <div className="mt-6 flex flex-wrap gap-2 rounded-xl border border-white/[0.08] p-4">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Team name" className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm" />
        <select value={type} onChange={(e) => setType(e.target.value)} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm">
          ...
          <tbody>
            {teams.map((t) => (
              <tr key={String(t.id)} className="border-b border-white/[0.04]">
                ...
                <td className="px-4 py-3">
                  <select value={String(t.status)} onChange={(e) => setStatus(String(t.id), e.target.value)} className="rounded border border-white/10 bg-black/20 px-2 py-1 text-xs">
