...
export function MobileShell({
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-lg flex-col bg-[#070b14] text-white">
      <header className="sticky top-0 z-20 border-b border-white/10 bg-[#070b14]/95 px-4 pb-3 pt-[max(0.75rem,env(safe-area-inset-top))] backdrop-blur">
        <div className="flex items-center justify-between gap-2">
          <div>
            <p className="text-[10px] uppercase tracking-[0.2em] text-sky-400/80">AI Campus Guardian</p>
            <h1 className="text-base font-semibold">{title || "Field Ops"}</h1>
          </div>
          <div
            className={cn(
              "flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-medium",
              connection === "OFFLINE" && "bg-amber-500/20 text-amber-200",
              connection === "SYNCING" && "bg-sky-500/20 text-sky-200",
      ...
      <nav
        className="fixed bottom-0 left-0 right-0 z-20 border-t border-white/10 bg-[#0a1020]/95 pb-[env(safe-area-inset-bottom)] backdrop-blur"
          ...
          {NAV.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex min-h-12 min-w-14 flex-col items-center justify-center gap-0.5 rounded-lg px-2 text-[10px]",
