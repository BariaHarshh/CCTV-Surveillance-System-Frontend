    ...
    <SuperAdminShell user={user}>
      ...
      {loading ? (
        ...
        <div className="mt-6 space-y-2">
          {flags.map((f) => (
            <div key={f.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 p-4">
              <div>
              ...
              <button
                type="button"
                onClick={() => toggle(f)}
                className={`rounded-lg px-3 py-1.5 text-xs font-medium ${f.enabled ? "bg-accent/20 text-accent" : "border border-white/10 text-muted"}`}
              >
...
export function AnnouncementsClient({ user }: { user: SafeUser }) {
  ...
  return (
    <SuperAdminShell user={user}>
      ...
      <form onSubmit={create} className="mt-6 rounded-2xl border border-white/10 bg-surface/50 p-6">
        <div className="grid gap-3">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title" className="rounded-xl border border-white/10 bg-background px-3 py-2 text-sm" required />
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Message" rows={3} className="rounded-xl border border-white/10 bg-background px-3 py-2 text-sm" required />
          <select value={type} onChange={(e) => setType(e.target.value)} className="rounded-xl border border-white/10 bg-background px-3 py-2 text-sm">
    ...
    <SuperAdminShell user={user}>
      ...
      <section className="mt-6 rounded-2xl border border-white/10 bg-surface/50 p-6">
        ...
        <textarea
          ...
          rows={3}
          className="mt-4 w-full rounded-xl border border-white/10 bg-background px-3 py-2 text-sm"
        />
        <div className="mt-4 flex gap-2">
          ...
          <button type="button" onClick={() => setEnabled(false)} className="rounded-xl border border-white/10 px-4 py-2 text-sm">
            Disable
          </button>
        </div>
      </section>
    ...
    <SuperAdminShell user={user}>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {(["FULL", "DATABASE", "CONFIGURATION"] as const).map((t) => (
          <button key={t} type="button" onClick={() => create(t)} className="rounded-xl border border-white/10 px-3 py-1.5 text-xs hover:border-accent/40">
            Run {t} backup
          </button>
        ))}
      </div>
...
export function PlatformIncidentsClient({ user }: { user: SafeUser }) {
  ...
  return (
    <SuperAdminShell user={user}>
      ...
      <form onSubmit={create} className="mt-6 rounded-2xl border border-white/10 bg-surface/50 p-6">
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Incident description" rows={2} className="w-full rounded-xl border border-white/10 bg-background px-3 py-2 text-sm" required />
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <select value={severity} onChange={(e) => setSeverity(e.target.value)} className="rounded-xl border border-white/10 bg-background px-3 py-2 text-sm">
      ...
      {loading ? (
        ...
        <div className="mt-6 space-y-3">
          {incidents.map((i) => (
            <div key={i.id} className="rounded-xl border border-white/10 p-4">
