  ...
  return (
    <SuperAdminShell user={user}>
      <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="mt-8 rounded-2xl border border-white/[0.08] bg-surface/60 p-6 text-left">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          ...
          <Link href={`/super-admin/organizations/${id}/admins/new`}
            className="rounded-full border border-white/[0.08] px-6 py-2.5 text-sm">Create Administrator</Link>
          <Link href="/super-admin/organizations"
            className="rounded-full border border-white/[0.08] px-6 py-2.5 text-sm">Back to Organizations</Link>
        </div>
      </motion.div>
    </SuperAdminShell>
  );
